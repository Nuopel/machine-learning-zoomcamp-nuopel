...
# ============================================================
# using LinearRegression_te to avoid Xboost and randomforest 1G image for demo docker
model = ['LinearRegression_te',
         'RandomForest_te',
         'Ridge_te',
         'XGBoost_te',
         'best_model_te']
index= 0
try:
    with open(f'./models/trained/{model[index]}.pkl', 'rb') as f_in:
        model = joblib.load(f_in)

    print("✅ Model loaded successfully")
except FileNotFoundError:
    print(f"⚠️ Warning: {model[index]} not found.")
    model = None
